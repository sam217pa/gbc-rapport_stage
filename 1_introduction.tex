...
\afterpage{%
  ...
  \caption[Mécanismes moléculaires de conversion génique]{\textbf{Mécanismes de
    ...
    séquence sauvage receveuse, une cassette de résistance à la kanamycine
    (KanR) et une zone très recombinogène (Ancre) qui permet d'augmenter la
    ...
    (\textsf{c}). La recombinaison homologue permet de réparer ces cassures en
    utilisant l'ADN exogène comme matrice. Le réappariemment (\textsf{d}) avec
    le brin parental conduit à la formation d'un hétéroduplex (zone grisée)
    entre le donneur (en bleu) et le receveur (en rouge). Les mésappariemments
    au sein de cet hétéroduplex sont réparés (\textsf{e}), du donneur vers le
    receveur dans la région convertie, du receveur vers le donneur dans la
    région conservée.
    \\
