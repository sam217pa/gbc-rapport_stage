\begin{center}
  ...
  \begin{tikzpicture}[scale=0.7, line width = 2pt, join=round, allow upside down]
    \begin{scope}[shift={(-3.5, 12)}]
      ...
      \draw[transition] [->] (1.5, 0) -- (2.7, 0) node[below, midway, font=\scriptsize] {PCR};
    \end{scope}

    \begin{scope}[shift={(-0.8, 10.7)}]
	    \draw[Gray]  (1.2, 0) node {\scriptsize +};
      \draw[plasmid_col, rounded corners = 1ex, thick]
      (0, 0) node[right, font=\tiny\sffamily] {T}
      -- ++(-0.5, 0)
      -- ++(0, -0.5)
      -- ++(3.5, 0) node[below, midway, font=\scriptsize] {pGEM-T}
      -- ++(0, 0.5)
      -- ++(-0.5, 0) node[left, font=\tiny\sffamily] {T};
    \end{scope}


    \begin{scope}[shift={(0, 11)}, line width = 1 pt]
    ...
    \draw[transition] [->] (2, 12) -- ++(1, 0)
    % node[above, midway]
    % {\includegraphics[width=0.1\textwidth, angle = 90]{schema/construct-img/ligation.png}}
    node[below, midway, font=\scriptsize]
    {Ligature};

    \begin{scope}[shift={(4.5, 12.5)}, scale = 0.8]
	    %% plasmide pGEM-T avec construction, avant infusion
      \draw[plasmid_col, rounded corners = 1ex, thick] (-0.8, -1) rectangle (2, 0);
      \draw[gs_don_col] (0, 0) -- (1, 0);
      \draw[am1_col] (0.8, 0) -- (1, 0) node[font =\scriptsize] {\(|\)} node[above, font=\scriptsize\itshape] {SpeI};
      \draw[am3_col] (1.0, 0) -- ++(0.2, 0);
    \end{scope}


    \draw[transition, out = 0, in = 90] [->] (7, 12) to ++(2, -0.5);
    \node[above, font=\scriptsize, Gray] at (8, 12) {Digestion par \emph{SpeI}};
    ...
    \draw[transition] [->] (14, 9.5) -- ++(0, -1) node[legende, midway, left] {Ligature \\ InFusion};
    ...
    \draw[transition] [->] (14, 7) -- (14, 6)
    node[legende, midway, left]
    {Transformation et\\extraction Plasmidique};
    ...
    \begin{scope}[shift={(-5, 1)}]
      ...
      \draw[Gray, dotted, thin] [->] (2, -0.5) -- ++(3, 0) node[above, midway, font=\tiny] {Homologues} -- ++(0.5, +0.4);
      \draw[Gray, dotted, thin] [->] (2, -0.5) -- ++(3, 0) -- ++(0.5, -0.4);
      \draw[Gray, dotted, thin] [->] (2, -0.5) -- ++(6, 0) -- ++(0.5, +0.4);
      \draw[Gray, dotted, thin] [->] (2, -0.5) -- ++(6, 0) -- ++(0.5, -0.4);
    ...
    \node[text width=6.5cm, legende text, align = left] at (14, -4) {%
      % 14 lignes possibles
      Les constructions linéarisées sont intégrées par recombinaison homologue
      entre le génome d'\emph{Acinetobacter} en phase exponentielle de
      croissance et la construction intégrée sous forme simple brin. Les
      cassures doubles-brins occasionnées aléatoirement au cours de la
      réplication au locus d'intérêt entraînent la résection de la région en
      \(3'\) du site de cassure. Cette résection est réparée par synthèse
      utilisant une matrice d'ADN homologue. En cultivant les transformants sur
      un milieu sélectif, nous avons sélectionné les clones qui ont utilisé la
      construction plasmidique pour matrice, ayant intégré la cassette de
      résistance à la kanamycine.

      Au cours du réappariemment des brins, un hétéroduplex~\tikz[scale =
      0.2]{\draw[Gray, thick, densely dotted, fill = Gray, opacity = 0.3] (0, 0)
        rectangle (1.618,1);} se forme : les brins homologues ne sont pas
      rigoureusement complémentaires. Le donneur \tikz[baseline = -0.5ex, very
      thick, scale = 0.3]{\draw[gs_don_col] (0, 0) -- ++(1, 0);} introduit des
      mésappariemments avec la séquence du receveur~\tikz[baseline = -0.5ex,
      very thick, scale = 0.3]{\draw[gs_rec_col] (0, 0) -- ++(1, 0);} ; ils
