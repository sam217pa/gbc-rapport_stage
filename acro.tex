% class `abbrev': abbreviations:
\DeclareAcronym{gc}{
  short = GC\% ,
  long  = Contenu en GC ,
  class = abbrev
}
\DeclareAcronym{gbgc}{
  short = gBGC ,
  long  = GC--biased gene conversion : conversion génique biaisée vers GC ,
  class = abbrev
}
\DeclareAcronym{dsb}{
  short = DSB ,
  long  = Double strand break : cassures doubles brins ,
  class = abbrev
}
\DeclareAcronym{mmr}{
  short = MMR ,
  long  = Mismatch repair : machinerie de réparation des mésappariemments ,
  class = abbrev
}
\DeclareAcronym{ber}{
  short = BER ,
  long  = Base Excision Repair ,
  class = abbrev
}
\DeclareAcronym{ner}{
  short = NER ,
  long  = Nucleotide Excision Repair ,
  class = abbrev
}
\DeclareAcronym{har}{
  short = HAR ,
  long  = Human Accelerated Regions : région accélérée spécifique à la lignée humaine ,
  class = abbrev
}
\DeclareAcronym{sdsa}{
  short = SDSA ,
  long  = Synthesis-Dependent Strand-Annealing ,
  class = abbrev
}
\DeclareAcronym{lb}{
  short = LB ,
  long  = milieu riche de Luria \& Bertani ,
  class = abbrev
}
\DeclareAcronym{vsp}{
  short = VSP ,
  long  = Very Short Patch repair : réparations courtes portées ,
  class = abbrev
}